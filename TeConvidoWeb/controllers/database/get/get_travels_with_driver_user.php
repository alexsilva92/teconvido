...
if($login != NULL){
        $sql = "SELECT t.id, t.origin,t.destination,t.dateTravel,t.dateCreation ".
               "FROM Travel t,TravelWithDriver d ".
               "WHERE t.id = d.travel AND d.driver = :user AND t.dateTravel >= CURDATE() ".
               "ORDER BY t.dateTravel";

        $db = connect($host, $user, $password, $database);
        if($db != NULL){
            $ps = $db->prepare($sql);
            $ps ->bindParam(':user', $login);
            $ps->execute();
